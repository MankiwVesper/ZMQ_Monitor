...
class SubCANThread(QObject):
    ...
    def _sub_zmq_can(self):
        # 订阅主题为 CanServerData 的ZMQ数据，保存到队列中
    ...
    def sub_can_data(self):
        """启动保存CAN数据和获取CAN数据的子线程"""
        # daemon=True，当主线程退出时，子线程立即退出
        sub_can_thread = threading.Thread(target=self._sub_zmq_can, daemon=True)
...
class MainWindow(QMainWindow, Ui_MainWindow):
    ...
    def __init__(self, parent=None):
        ...
        self.setupUi(self)
        self.setWindowTitle("CAN-ZMQ-Monitor (V1.1 2024/09/13)")
    ...
    def update_sub_info(self, zmq_point, is_sub):
        ...
        if hasattr(self, "my_sub_data_thread"):
            ...
            self.my_sub_data_thread.zmq_suber.get_update_sub_info(zmq_point, is_sub)
    ...
    def sub_data_start(self):
        ...
        self.my_sub_data_thread = SubCANThread(zmq_points)
        self.my_sub_data_thread.moveToThread(self.sub_data_thread)

        # 订阅数据的子线程开始之后，调用获取数据的函数
        self.sub_data_thread.started.connect(self.my_sub_data_thread.sub_can_data)

        # 收到can_message信号之后，将数据插入到 TableView中
        self.my_sub_data_thread.can_message.connect(self.add_new_data)

        # 停止订阅后 删除相关线程
        self.my_sub_data_thread.stop_get_message.connect(self.sub_data_thread.quit)
        self.my_sub_data_thread.stop_get_message.connect(self.my_sub_data_thread.deleteLater)
        self.sub_data_thread.finished.connect(self.sub_data_thread.deleteLater)

        # 更新提示信息
        self.my_sub_data_thread.sub_state_bad.connect(self.bad_sub_state_update)
        self.my_sub_data_thread.sub_state_good.connect(self.good_sub_state_update)
        self.my_sub_data_thread.sub_warning.connect(self.warnings_update)
    ...
    def suspend_sub_data(self):
        ...
        self.my_sub_data_thread.can_message.disconnect()  # 解除绑定 抛出数据信号和写入TableView中的槽函数
    ...
    def continue_sub_data(self):
        ...
        self.my_sub_data_thread.can_message.connect(self.add_new_data)  # 重新绑定抛出数据信号和写入table view 的槽函数
    ...
    def reset_sub_data(self):
        ...
        # 触发订阅子线程中的停止事件
        self.my_sub_data_thread.stop_event.set()
